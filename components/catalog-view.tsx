...
import { catalogConfigs, products, type CatalogSlug, type Product } from "@/data/catalog";
...
type NavPanel = "shop" | "search" | "bag" | "account" | null;
...
function ProductCard({
  ...
  const productHref = product.id === "pink-bloom" ? "/products/pink-bloom" : undefined;
  ...
  return (
    <article className="catalog-card" id={product.id}>
      ...
      >
        ...
        {productHref && <a className="catalog-card-pdp-link" href={productHref} aria-label={`View ${product.name}`} />}
        ...
        <div className={`catalog-card-controls${wished ? " has-wish" : ""}`}>
          ...
          ><span aria-hidden="true">{wished ? "♥" : "♡"}</span></button>
      ...
      <div className="catalog-card-copy">
        <div className="catalog-card-heading">
          <h2>{productHref ? <a href={productHref}>{product.name}</a> : product.name}</h2>
...
export default function CatalogView({ slug }: { slug: CatalogSlug }) {
  ...
  const options = useMemo(() => ({
    ...
    collections: [...new Set(baseProducts.map((product) => product.collection))],
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (slug === "all-jewellery") return [...result].sort((a, b) => featuredOrder.indexOf(a.id) - featuredOrder.indexOf(b.id));
    return result;
    ...
    <main className="catalog-page">
      <header className="site-header catalog-header">
        ...
        <nav className="desktop-nav desktop-nav-right" aria-label="Client navigation">
          ...
          <button onClick={() => setNavPanel("bag")}>Bag <sup>0</sup></button>
        </nav>
        <button className="mobile-bag" aria-label="Shopping bag" onClick={() => setNavPanel("bag")}><BagIcon /><sup>0</sup></button>
      </header>
        ...
        {filteredProducts.length ? (
          <>
            <div className={`catalog-grid catalog-grid-${view}`}>
              {filteredProducts.slice(0, visibleCount).map((product, index) => <ProductCard product={product} priority={index < 4} wished={wishlist.has(product.id)} onToggleWish={() => toggleWishlist(product.id)} key={product.id} />)}
            </div>
      ...
      <section className="catalog-concierge">
        ...
        <a className="button button-light" href="mailto:hello@rosehk.com">Start a conversation</a>
      </section>
      ...
      {navPanel && <div className="nav-overlay" role="dialog" aria-modal="true" aria-label={`${navPanel} menu`}><button className="nav-backdrop" onClick={() => setNavPanel(null)} aria-label="Close menu" /><div className="nav-sheet"><div className="nav-sheet-top"><button className="nav-close" onClick={() => setNavPanel(null)} aria-label="Close menu"><span /><span /></button><a className="wordmark" href="/" aria-label="ROSÉ Diamonds home"><BrandLogo /></a><button className="nav-sheet-bag" onClick={() => setNavPanel("bag")} aria-label="Shopping bag"><BagIcon /><sup>0</sup></button></div>
        {navPanel === "shop" && <div className="nav-shop-layout catalog-nav-shop"><div className="nav-shop-intro"><small>Shop</small><p>Fine jewellery chosen by piece, collection or feeling.</p></div><div className="nav-shop-row"><div className="nav-column nav-featured"><small>Discover</small><a href="/collections/all-jewellery">All Jewellery <MenuArrowIcon /></a><a href="/collections/rose-dopamine">ROSÉ Dopamine <MenuArrowIcon /></a><a href="/#most-wanted">Most Wanted <MenuArrowIcon /></a></div><div className="nav-column"><small>Jewellery</small>{CATEGORY_LINKS.slice(1, 5).map(([label, href]) => <a href={href} key={href}>{label} <MenuArrowIcon /></a>)}</div><div className="nav-column"><small>Services</small><a href="/#design-your-piece">Design Your Piece <MenuArrowIcon /></a><a href="/#concierge">Concierge <MenuArrowIcon /></a></div></div></div>}
        {navPanel === "search" && <div className="nav-search-layout"><label htmlFor="catalog-search">What are you looking for?</label><div><input id="catalog-search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search jewellery" /><span>{searchResults.length} results</span></div><nav><small>Pieces</small>{searchResults.map((product) => <a href={`/collections/${product.collection === "ROSÉ Dopamine" ? "rose-dopamine" : product.category.toLowerCase()}#${product.id}`} key={product.id}>{product.name}</a>)}</nav></div>}
        {navPanel === "bag" && <div className="nav-bag-layout"><small>Your bag</small><h3>Your bag is currently empty.</h3><p>Discover pieces chosen to be worn your way.</p><a className="button button-dark" href="/collections/all-jewellery">Explore jewellery</a></div>}
